...
class SpotifyFetcher:
    ...
    async def get_current_user_id(self) -> str:
        data = await self._client.get("/me")
        return str(data["id"])

    async def create_playlist(self, user_id: str, name: str) -> str:
        """Create a new Spotify playlist and return its Spotify ID."""
        data = await self._client.post(
            f"/users/{user_id}/playlists",
            json={"name": name, "public": False, "description": "Generated by TasteRanker"},
        )
        return str(data["id"])

    async def add_tracks_to_playlist(self, playlist_id: str, track_uris: list[str]) -> None:
        """Add tracks to a Spotify playlist in batches of 100 (Spotify API limit)."""
